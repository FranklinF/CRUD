Vue.component('voidcoupons-form-header', {
    props:['isformvalid'],
    computed: {
        isDisabled() {
    ...
    <div class="m-portlet__head">
        ...
        <div class="m-portlet__head-wrapper">
            ...
            <div class="m-portlet__head-tools">    
                <a href="#" v-on:click="$emit('voidcouponformcancel',$event)" class="btn btn-secondary m-btn m-btn--icon m-btn--wide m-btn--md m--margin-right-10">
                    <span>
                        <i class="la la-arrow-left"></i>
                        <span>Back</span>
                    </span>
                </a>                       
                <button v-on:click="$emit('voidcouponformsave',$event)" type="button" class="btn btn-primary m-btn m-btn--custom" id="saveBtn" :disabled="isDisabled">Void Coupon</button>
            </div>
        </div>
    </div>
    `
...
Vue.component('voidcoupons-form-component', {
    props:['primaryinput'],
    data: function() {
        return {
            couponList: [],
    ...
    methods: {
        ...
        submitForm:function(){
            var vm = this;
            var form = $('#m_form_voidcoupon_new');
            var btn = $('#saveBtn');
            ...
            axios.put('/loyalty/coupons/serials/void', data)
                .then(response => {
                    console.log("void Serials ", response);
                    this.resetData();
                    this.serials = [];
                    this.getSerials();
                    this.$emit('showtoast',{'msg':'voided Success','type':'success','event':this.$event});
                    this.$emit('voidcouponformsave',{'voidcoupon':this.voidcoupon,'event':this.$event});
                })
                .catch(err => {
                    ...
                    this.$emit('showtoast',{'msg':'void Failed ' + err.response.data.error_msg,'type':'error','event':this.$event})
                })
        ...
        cancelForm:function(){
            this.resetData();
            this.$emit('voidcouponformcancel',{'voidcoupon':this.voidcoupon,'event':this.$event})
    ...
    template: `
    <div class="m-grid__item m-content-voidcoupon-form d-none" id="m-voidcoupon-form" >
        <div class="m-content">
            <div class="row">
                <div class="col-lg-12">
                    <!--begin::Portlet-->
                    <div class="m-portlet m-portlet--last m-portlet--head-lg m-portlet--responsive-mobile" id="main_portlet">
                    <voidcoupons-form-header v-bind:isformvalid="isValidForm"
                            @voidcouponformcancel="cancelForm()"
                            @voidcouponformsave="submitForm()"
                    ></voidcoupons-form-header>
                        <div class="m-portlet__body">
                            <form class="m-form m-form--fit m-form--label-align-right" id="m_form_voidcoupon_new">
                                <div class="m-portlet__body">
                                    <div class="row">
                                        <div class="col-xl-12">                                            
